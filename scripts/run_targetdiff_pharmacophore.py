#!/usr/bin/env python3
"""TargetDiff + Pharmacophore-Guided Generation — Proof-of-Concept.

Based on run_targetdiff_native_guided.py, adapted for pharmacophore feature
points extracted from the reference ligand.

Modes: unguided (native), hard_fix (overwrite anchors to pharm points),
kinematic (CoM-attraction toward pharm points).
"""
...
from guidance.latent_guidance import (
    ...
    PHARMACOPHORE_COMPAT_MATRIX,
    PHARMACOPHORE_TYPE_ORDER,
)
from guidance.kinematic_anchor import KinematicScheduler
from site_detection.pharmacophore_sites import (
    extract_pharmacophore_sites,
    select_pharmacophore_anchors,
    get_pharmacophore_type_index,
)
...
# ═══════════════════════════════════════════════════════════
# Monkey-patch sample_diffusion for pharmacophore guidance
# ═══════════════════════════════════════════════════════════

def make_guided_sample_diffusion(mode, anchor_indices, anchor_targets,
                                  site_energy, lambda_max):
    """Return patched model.sample_diffusion with pharmacophore guidance."""
    ...
    def guided_sample_diffusion(self, protein_pos, protein_v, batch_protein,
        ...
        for step_i, i_val in enumerate(time_seq):
            ...
            with torch.no_grad():
                ...
                # ── PHARMACOPHORE GUIDANCE INJECTION ──
                if mode == "hard_fix" and anchor_targets is not None:
                    for mol_i in range(num_graphs):
                        ...
                        na = len(mol_idxs)
                        for k, ai in enumerate(anchor_indices):
                            if 0 <= ai < na and k < anchor_targets.shape[0]:
                                ligand_pos_next[mol_idxs[ai]] = anchor_targets[k].to(ligand_pos_next.device)
...
def compute_pharm_occ(mols_data, pharm_site_map, threshold=2.5):
    """Compute pharmacophore site occupancy rate.

    A molecule is counted as 'occupied' if at least one heavy atom is within
    threshold of at least one pharmacophore site.
    """
...
def main():
    parser = argparse.ArgumentParser(description="TargetDiff pharmacophore-guided generation PoC")
    parser.add_argument("--pocket", required=True, choices=["3mfw", "6o4x", "2gni"])
    parser.add_argument("--mode", default="all",
                        choices=["unguided", "hard_fix", "kinematic", "all"])
    parser.add_argument("--n-samples", type=int, default=50)
    parser.add_argument("--output-dir", default="experiments/targetdiff_pharmacophore")
    ...
    # ── Reference ligand and pharmacophore site extraction ──
    ref_ligand_path = f"/root/autodl-tmp/data/PDB/P-L/{year}/{pocket}/{pocket}_ligand.sdf"
    pharm_map = extract_pharmacophore_sites(ref_ligand_path, protein_id=pocket,
                                             ligand_id=f"{pocket}_ligand")
    n_sites = len(pharm_map["sites"])
    print(f"{pocket}: {n_sites} pharmacophore features")
    for s in pharm_map["sites"]:
        print(f"  [{s['site_type']:15s}] @ ({s['center'][0]:6.1f}, "
              f"{s['center'][1]:6.1f}, {s['center'][2]:6.1f})")

    # ── Anchor selection from reference ligand ──
    anchor_indices, anchor_positions = select_pharmacophore_anchors(
        ref_ligand_path, pharm_map, max_anchors=args.max_anchors)
    print(f"Selected {len(anchor_indices)} anchor atoms from reference ligand")
    print(f"  indices: {anchor_indices}")

    # ── Pharmacophore site energy ──
    se = SiteCompatibilityEnergy(sigma_distance=3.0)
    se.compatibility_matrix = PHARMACOPHORE_COMPAT_MATRIX.clone()
    se.n_envs = len(PHARMACOPHORE_TYPE_ORDER)
    if n_sites > 0:
        sc = torch.tensor([s["center"] for s in pharm_map["sites"]], dtype=torch.float32)
        env_idx = torch.tensor(
            [get_pharmacophore_type_index(s["site_type"]) for s in pharm_map["sites"]],
            dtype=torch.long
        )
        se.register_sites(sc, env_idx)
    se.to(args.device)
    ...
    # ── Anchor targets for hard_fix ──
    anchor_t = torch.tensor(anchor_positions, dtype=torch.float32) if len(anchor_indices) > 0 else torch.zeros(0, 3)
    ...
    for mode in modes:
        print(f"\n{'='*50}")
        print(f"[{pocket}] {mode} ({args.n_samples} mols, {args.num_steps} steps)")
        print(f"{'='*50}")

        at = anchor_t if mode == "hard_fix" else None
        site_e = se if mode == "kinematic" else None
        patched_fn = make_guided_sample_diffusion(mode, anchor_indices, at,
                                                   site_e, args.lambda_max)
        ...
        pharm_occ = compute_pharm_occ(
            [{"pos": p, "v": v} for p, v in zip(positions, types)], pharm_map
        )

        print(f"  Time: {elapsed:.0f}s ({elapsed/max(len(positions),1):.1f}s/mol)")
        print(f"  Valid: {len(valid)}/{len(positions)} ({len(valid)/max(len(positions),1):.0%})")
        print(f"  PharmOcc: {pharm_occ:.1%}")
        ...
        torch.save({
            ...
            "valid": len(valid), "pharm_occ": pharm_occ,
            "pharm_site_map": pharm_map,
        }, mode_dir / "results.pt")
        ...
        summaries[mode] = {
            ...
            "n_pharm_sites": n_sites,
            "time": elapsed,
        }

    # ── Summary ──
    print(f"\n{'='*50}\nRESULTS: {pocket} (pharmacophore PoC)\n{'='*50}")
    print(f"{'Condition':<15} {'PharmOcc':>10} {'Valid':>10}")
    print("-" * 35)
    for mode in modes:
        s = summaries[mode]
        print(f"{mode:<15} {s['pharm_occ']:>9.1%} "
              f"{s['n_valid']:>5}/{s['n_total']:<5}")

    with open(outdir / "summary.json", "w") as f:
        json.dump({
            "pocket": pocket,
            "generator": "TargetDiff",
            "experiment": "pharmacophore_poc",
