...
def pairwise_concat(x, x_mask, y, y_mask, *pairwise_features):
    """
    Return flatten tensor containing only non-padding elements.
    ...
    xy_pad = torch.zeros_like(xy)

    x1 = torch.cat(
        (x.unsqueeze(2), xy_pad[:, : x.shape[1]].unsqueeze(2)), -1
    )  # B x C x 1 x feature_dim*2
    x2 = torch.cat(
        (xy_pad.unsqueeze(1), xy.unsqueeze(1)), -1
    ...
    # x_cat = torch.zeros_like(x_cat_tmp)
    # x_cat[pair_mask.expand(pair_mask.shape[:-1] + x_cat_tmp.shape[-1:]) ] = x_cat_tmp.masked_select(pair_mask) # x_cat_size B x C x C x (feature_dim*2 + len(pairwise_features))
    # return x_cat
